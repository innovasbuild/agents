...
- [ ] **Step 6: Write a minimal `cola-client.tsx` so the tree compiles**

`page.tsx` importa `ColaClient`, que recién se escribe completo en la Task 6.
Para que el árbol compile al cerrar esta task, creá la versión de solo lectura:
sin acciones, sin Realtime, sin estado. La Task 6 la reemplaza entera.

```tsx
// app/[tenant]/cola/cola-client.tsx
"use client";
...
export function ColaClient({ rows }: {
	slug: string;
	tenantId: string;
	...
	if (rows.length === 0) {
		return (
			<p className="text-muted-foreground">
				No hay piezas esperando. Pedile al agente que arme la cola desde el chat.
			</p>
		);
	}
...
Expected: sin errores. (Si falla por el import de `ColaClient`, revisá que el
nombre exportado sea exactamente ese.)

- [ ] **Step 8: Commit**

```bash
git add lib/outreach/cola-query.ts tests/cola/cola-query.test.ts app/\[tenant\]/cola/page.tsx app/\[tenant\]/cola/cola-client.tsx
...
**Files:**
- Replace: `app/[tenant]/cola/cola-client.tsx` (la Task 5 dejó una versión de solo lectura; esta la reemplaza entera)
