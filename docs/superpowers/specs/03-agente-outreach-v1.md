...
4. **Escucha y follow-ups:** primero S2 en producción (§13.1); después scopes de Gmail y hook, `listen.ts`, `read_replies`, los dos schedules, F6.5 y resumen de sesión. Junto con la lectura de Gmail (S2) entra acá la reconciliación automática de piezas trabadas en `approved` (§16): buscar por `Message-ID` (`qi-<queueItemId>@...`) si el mail salió y cerrar la pieza sola, en vez de dejarla solo visible como `trabada` en `list_queue`.
