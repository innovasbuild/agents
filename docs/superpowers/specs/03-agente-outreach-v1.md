...
| D11 | **Research por tool común, sin subagente** (plan B de S4, §13.1): `research_account` redacta la ficha con `generateText` + `Output.object` y una sola tool propia, `leer_pagina` (§6.3) | La workflow tool con `ctx.agent("researcher", …)` no corre en eve 0.54.2-0.54.5 con el agente en `agents/outreach/`: el bundle registra el workflowId sin el prefijo del agente (§13.1 S4 y §16). La tool común devuelve la ficha tipada en el momento; llamar al subagente como tool devolvería un recibo en segundo plano, no la ficha |
...
### 6.3 Research (`research_account`)

Sin subagente (plan B de S4, §13.1): `research_account` es una tool común que llama `generateText` + `Output.object({ schema: fichaSchema })` con el modelo de `config.outreach.models.researcher`, el procedimiento genérico de research del canon como system (`RESEARCH_SYSTEM` en `lib/outreach/services/research-run.ts`: qué produce y vende, cómo gana plata, qué compra, qué se rompe si crece, gap declarado contra demostrable, regla de ancla) y una sola tool, `leer_pagina`, hasta 8 páginas.
`leer_pagina` usa `fetchPublicPage` (`lib/outreach/web-page.ts`) con protección SSRF: solo http(s) a puertos estándar, sin credenciales, host que resuelve solo a IPs públicas antes de cada request y de cada redirección (hasta 3), timeout de 10 s, HTML o texto, cortado a 1 MB y 20.000 caracteres.
...
|---|---|---|---|---|
| `import_contacts(csv)` | tool | — | inserta `contacts` | `contact_key`; G1 contra el CRM; claim por DB y autoría del CRM. Veredicto por fila: `nuevo` (se inserta, claim libre), `ya_existia` (no cambia nada; también aplica a un contacto libre ya cargado), `claim_ajeno`, `sin_email` e `invalida` (estas tres no se insertan). Evento `contacto_importado`. El ICP lo juzga el modelo con el canon |
| `research_account(domain, name?)` | tool | — | upsert `accounts` | Ficha vigente → la devuelve sin costo. Si no: `generateText` + `Output.object` con `leer_pagina` (§6.3), `sanitizeFicha`, rechaza sin ningún hecho con URL (`sin_ancla`); un fallo del modelo o la red es `research_fallido`. Evento `investigado` |
...
| `list_queue()` | tool | — | ninguno | Piezas `pending` **y `approved`** del ejecutor con letra, destinatario, asunto, cuerpo y gate. Las `approved` son las trabadas entre el claim de `send_email` y el envío (§16): salen con `trabada: true` y su `approvedAt` para que el ejecutor las revise en Gmail antes de tocar nada |
...
5. Envío por Gmail: texto plano, BCC de `config.outreach.bcc`, `Message-ID: <qi-<id>@<dominio del remitente>>`, y para follow-ups `threadId`, `In-Reply-To` y `References` con `reply_to_message_id` (`lib/gmail/mime.ts`). 401 → pieza a `pending` y `ctx.requireAuth`, como hoy. Otro error → `failed` + evento `envio_fallido`. Gmail reescribe este `Message-ID` propio al entregar (S2, §13.1): no sirve para encontrar el mail después (la conciliación va por `gmail_thread_id`, §8.2). Se conserva igual porque no molesta; a criterio de quien retome esto, podría sacarse por no aportar nada.
...
8. Registro en la base: evento `envio` (payload: `queue_item_id`, `gmail_message_id`, `kind`, `hook`, `vector`, uso de tokens de la redacción); en `contacts`: `touches + 1`, `last_touch_at`, `first_touch_at` y `gmail_thread_id` si es `msg1`, `stage = msg1_enviado` si es `msg1`, `next_step_at = nextFollowup(...)`. *Entrega 3:* el uso de tokens de la redacción todavía no se guarda en `envio`; queda pendiente para la Entrega 4.
9. Registro en el CRM (si hay capacidad `crm`), por `CrmAdapter` (§9): upsert con las 10 propiedades en la misma llamada (`outreach_fecha_msg1` solo en `msg1`), nota `[out · <kind> · email · <vector> · <hook>]` con el texto exacto, cierre de la task anterior y task nueva en `next_step_at` si hay siguiente toque. Si falla: evento `crm_sync_pendiente` y la tool devuelve `ok: true` con aviso. **Nunca se reenvía por un error del CRM.** *Entrega 3:* en el envío van 9 propiedades, no 10: `outreach_fecha_respuesta` la escribe la escucha (§8.2, Entrega 4).
...
En `lib/outreach/listen.ts` (con I/O inyectado para testear). La conciliación es por `gmail_thread_id` guardado en el envío (§7.2), no por `rfc822msgid:` con un `Message-ID` propio: Gmail lo reescribe (S2, §13.1).
...
- **Aprobadas colgadas:** piezas `approved` hace más de 15 minutos → búsqueda por destinatario y ventana de fecha (`in:sent to:<email> newer_than:10d`; Gmail reescribe el `Message-ID` propio, `rfc822msgid:` no encuentra nada, S2). Encontrada → `sent` y los pasos 8 y 9 de §7.2. No encontrada → `failed` con `error = "sin_confirmacion"`.
...
| S2 | **PASS con salvedad.** Corrido en producción el 2026-09-15 con el grant `[gmail.send, gmail.readonly]` ya autorizado (consentimiento nuevo, sin problema). Los scopes concedidos se leen con `POST https://oauth2.googleapis.com/tokeninfo` (token en el body). La lectura de Gmail funciona. **Gmail reescribe el `Message-ID`** de lo enviado: buscar por el propio (`rfc822msgid:`) da 0 resultados; el fallback `in:sent to:<email> newer_than:10d` sí sirve como conciliación alternativa | `s2.scopes`: `scopes [gmail.readonly, gmail.send], expires_in 2899`. `s2.messages.list`: `status 200, mensajes 3`. `s2.header_match`: `coincide false` (Gmail devolvió `<CACHZpToScqTcDDLqfAbv3cKJjAJU5iTB42tWKP6v0k7VVfDUdg@mail.gmail.com>` en vez de `<qi-992a6edc-dd9b-4478-b290-bdeb3a254ec8@innov.as>`). `s2.rfc822msgid`: 0 resultados. `s2.fallback`: `status 200, mensajes 1` | No hace falta buscar por Message-ID propio: el envío ya guarda `gmail_message_id`/`gmail_thread_id` de lo que devuelve Gmail, y la escucha concilia por `gmail_thread_id` (§8.2). El fallback por destinatario y fecha queda solo para el barrido de piezas trabadas en `approved` sin confirmación de envío (§8.3) |
...
4. **Escucha y follow-ups:** S2 ya corrido en producción, PASS con salvedad (§13.1); ahora scopes de Gmail y hook, `listen.ts`, `read_replies`, los dos schedules, F6.5 y resumen de sesión. Junto con la lectura de Gmail entra acá la reconciliación automática de piezas trabadas en `approved` (§16): buscar por destinatario y ventana de fecha (Gmail reescribe el `Message-ID` propio, `rfc822msgid:` no sirve, S2) si el mail salió y cerrar la pieza sola, en vez de dejarla solo visible como `trabada` en `list_queue`.
...
- **Grant de Gmail que "muere" (visto en S1 y S4; causa confirmada en S2):** Connect matchea el grant por el **conjunto exacto** de scopes, no por subconjunto — si ya se autorizó `[gmail.send, gmail.readonly]` y una tool pide solo `gmail.send`, no matchea y tira `UserAuthorizationRequiredError`, dejando ese conector sin token hasta que la persona completa el consentimiento nuevo. Corregido en código: `GMAIL_SCOPES` único en `lib/gmail/send.ts`. Operativamente: sumar un scope a futuro obliga a reautorizar a cada ejecutor, y entre el link y el consentimiento ese conector queda sin token.
...
- **Bug de eve con workflow tools en agentes anidados (`agents/<id>/`):** reportarlo upstream y no usar `defineWorkflowTool` hasta que esté arreglado.
...
- **Pregunta y aprobación en el mismo paso:** si el modelo llama `ask_question` y `send_email` en el mismo paso, eve 0.54.2 los junta en un lote (`harness/tool-loop.js`): la respuesta a la pregunta queda retenida hasta que se responde la aprobación, pero el cliente oculta la tarjeta de la pregunta al toque. Si el usuario responde "no" y después aprueba, el mail sale. En producción fueron turnos separados y la regla de instrucciones apunta a que no pase; la Entrega 3 tiene que agrupar en una sola tarjeta los pedidos del mismo turno y paso, o deshabilitar la pregunta mientras la aprobación de ese lote siga pendiente.
- **Tarjetas colgadas y el input bloqueado:** desde el fix del chat, el input principal se bloquea mientras haya un pedido pendiente o un error (eve resuelve el texto contra las opciones, `channel/resolve-text.js`). En eve 0.54.2 `turn.failed` y `session.failed` no limpian las partes en `approval-requested` del reducer: si el servidor ya descartó ese pedido, la tarjeta vuelve en cada recarga y responderla genera un mensaje de usuario sintético (`convertStaleResponsesToUserMessage`). Con la sesión muerta (409) el hilo queda inutilizable y hay que abrir otro. La Entrega 3 tiene que decidir si el cliente oculta pedidos de turnos fallidos o si se reporta a eve.
- **Pieza trabada en `approved` (Task 24):** `sendQueuedEmail` transiciona `pending → approved` antes de revalidar y enviar; si el turno se cancela, hace timeout o cae por un deploy entre ese claim y la escritura de `sent`, la pieza queda en `approved` para siempre — solo `sendQueuedEmail` puede sacarla de ahí y el índice único de pieza viva bloquea a ese contacto para nuevas piezas. No hay tool para destrabarla a mano: reencolar arriesgaría un segundo mail si el de Gmail sí había salido. Mitigación de la Entrega 3: `list_queue` muestra también las piezas `approved` con `trabada: true` y su `approvedAt`, para que el ejecutor las note y revise en Gmail antes de tocar nada.
