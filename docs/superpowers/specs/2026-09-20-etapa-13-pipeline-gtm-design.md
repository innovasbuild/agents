...
|---|---|---|---|
| S1 | ¿`evaluate()` devuelve `confidence` en `providerMetadata.typesafe` o dentro de cada answer? La doc de Vercel y la de TypeSafe difieren | `icp-scoring` | **No verificado (2026-09-21):** no hay `AI_GATEWAY_API_KEY` disponible en este worktree. El script `scripts/spike-jev.mts` queda listo para correr en cuanto haya una forma segura de traer la key (por ejemplo desde el checkout principal, no con `vercel link` en este worktree). La Task 12 implementa `readScore`/`readNoul` leyendo de forma defensiva de los dos lugares posibles (`answers` y `providerMetadata`), así que no depende de que este spike se haya corrido. |
| S2 | ¿`evaluate()` devuelve `usage` y `providerMetadata.gateway.cost` con la misma forma que `generateText`? | La medición de esta etapa | **No verificado (2026-09-21):** no hay `AI_GATEWAY_API_KEY` disponible en este worktree. El script `scripts/spike-jev.mts` queda listo para correr en cuanto haya una forma segura de traer la key (por ejemplo desde el checkout principal, no con `vercel link` en este worktree). La Task 12 implementa `readScore`/`readNoul` leyendo de forma defensiva de los dos lugares posibles (`answers` y `providerMetadata`), así que no depende de que este spike se haya corrido. |
