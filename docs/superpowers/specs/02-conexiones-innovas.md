...
S1 a S6 se corrieron con el usuario en su terminal; el agente no vio llaves ni tokens. **S7 no se corrió en el spike:** exigía `tenantScopedConnect`, que no existía todavía. Se corrió en la verificación del criterio de cierre 4 (Task 14, 2026-09-14) y su resultado —una fuga real, ya con fix— está documentado en la fila S7 de abajo.
...
| S7 | **No al momento de correrlo — fuga confirmada.** En la verificación del criterio de cierre 4 (Task 14, 2026-09-14) el mismo usuario (Supabase auth id `482569a2-e560-487b-b4e4-0f006fb85490`), ya autorizado en `innovas`, **no volvió a pedir "Autorizar HubSpot"** en un tenant de prueba (`prueba-conexiones`, id `7522ac1b-4e10-409e-bbd8-52319cee9a65`) con su propio binding `crm`/`hubspot`: el agente devolvió resultados reales de HubSpot en el primer pedido. `tenantScopedConnect` (`lib/connectors/auth.ts`) ata el subject a `tenantId:userId` correctamente y se descartó como causa (ver evidencia). Causa raíz real: `ConnectionRegistryImpl.getClient()` (`node_modules/eve/dist/src/runtime/connections/registry.js`) cachea el cliente MCP conectado indexado **solo por `connectionName`**, y `connectionName("hubspot")` devolvía el string fijo `"crm"` para cualquier tenant (`multiple: false`). `McpConnectionClient` (`node_modules/eve/dist/src/runtime/connections/mcp-client.js`) resuelve el header `Authorization` una sola vez en el primer `connect()` y lo deja fijo en esa instancia cacheada. Con Fluid Compute reutilizando la misma instancia de función tibia entre sesiones, un cliente ya autenticado para un tenant podía quedar cacheado bajo `"crm"` y servirse tal cual a otro. **Fix:** `connectionName` ahora incluye `binding.id` para todo proveedor `authKind: "connect_oauth"` (`lib/connectors/providers.ts:64-79`), así el nombre de conexión — y la clave del caché de eve — nunca colisiona entre tenants | Reproducido a mano en producción (`agents-six-iota.vercel.app`) el 2026-09-14. Se descartó la hipótesis de Connect: replay del request exacto (`POST /v1/connect/token/mcp.hubspot.com%2Fhubspot` con el subject de `prueba-conexiones`, OIDC del proyecto) → `401 user_authorization_required` (Connect sí aísla por subject). Se descartó `instanceKey`: ya incorpora `binding.id` (`lib/connectors/catalog.ts`) y participa del caché de tokens de eve, pero no del caché de clientes MCP conectados de `registry.js`. Fix verificado con `npm test` (183 tests, incluye regresión "dos tenants ... nunca comparten nombre de conexión"), `npm run typecheck` y `npm run build`, y **re-verificado en producción el 2026-09-14**: mismo usuario y mismo tenant de prueba, el segundo tenant volvió a pedir "Autorizar HubSpot" y trajo sus propios datos tras autorizar | §14, `lib/connectors/providers.ts`, `lib/connectors/catalog.ts`, `tests/connectors/providers.test.ts`, `tests/connectors/catalog.test.ts` |
