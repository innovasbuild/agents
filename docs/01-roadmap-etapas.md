...
- [ ] **E3 · Enrichment y pieza (plan listo, sin implementar):** revelado de email con promoción de `contact_key` (Task 17), workflow `contact-enrichment` (Task 18), `verify-fact` (Task 19), `draft-queue` con cupo diario y reintento por fecha — enmienda D17 (Task 20), cierre contra producción (Task 21, depende de la Task 10/Apollo). Plan detallado en `docs/superpowers/plans/2026-09-20-etapa-13-pipeline-gtm.md`, agregado el 2026-09-22.
- [ ] **E4 · Pantallas (plan listo, sin implementar):** lecturas/escrituras de `/focos` (Task 22), la pantalla (Task 23), la bandeja "para revisar" (Task 24), columna de puntaje en `/contactos` (Task 25), baja del script de CLI (Task 26, depende de la Task 10/Apollo para cerrar contra producción).
